//
// Created by pavanmv on 10/07/21.
//
...
void TargetLocator::locate_white_ball(const sensor_msgs::Image& img) {
    ...
    if (count < max_ball_image_size) {
        if (count != 0){
            ...
            angular_z = std::max((float)-1, std::min((float)1, angular_z));

            //large angular_z not working along with large linear_x
            linear_x = std::min((double)1, range*x_factor);
